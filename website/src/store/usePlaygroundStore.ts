...
interface PlaygroundState {
  ...
  // LSP Actions
  setLspStatus: (status: 'disabled' | 'connecting' | 'connected' | 'error') => void
...
const initialFiles = INITIAL_DEMO.files.reduce(
  (acc, file) => {
    acc[file.name] = {
      ...file,
...
export const usePlaygroundStore = create<PlaygroundState>((set, get) => ({
  ...
  selectDemo: async (demoId) => {
    ...
    const newFiles = demo.files.reduce(
      (acc, file) => {
        acc[file.name] = {
          ...file,
          content: file.content || '',
  ...
  setLspClient: (client, worker) => set({ client, worker, lspStatus: 'connected' }),
