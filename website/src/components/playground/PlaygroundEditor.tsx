...
import { TYPEDOWN_THEME_DARK, TYPEDOWN_THEME_LIGHT } from '@/lib/typedown-theme'
...
export function PlaygroundEditor() {
  ...
  function handleEditorWillMount(monaco: Monaco) {
    ...
    if (!monaco.languages.getLanguages().some((l: { id: string }) => l.id === 'typedown')) {
    ...
    if (!window.__typedownSemanticTokensProviderRegistered) {
      logger.debug('[PlaygroundEditor] Registering Semantic Tokens Provider...')
      ...
      monaco.languages.registerDocumentSemanticTokensProvider('typedown', {
        ...
        provideDocumentSemanticTokens: async (
          ...
          try {
            ...
            logger.debug('[SemanticTokensProvider] Requesting tokens for:', uri)
      ...
      logger.debug('[PlaygroundEditor] Semantic Tokens Provider manually registered')
    } else {
      logger.debug('[PlaygroundEditor] Semantic Tokens Provider already registered, skipping')
    }
  ...
  function handleEditorDidMount(editor: MonacoTypes.editor.IStandaloneCodeEditor, monaco: Monaco) {
    ...
    window.__debugRefreshSemanticTokens = () => {
      ...
      const providers = (monaco.languages as any).DocumentSemanticTokensProvider
      ...
      editor.executeEdits('', [{ range: new monaco.Range(1, 1, 1, 1), text: ' ' }])
      editor.executeEdits('', [{ range: new monaco.Range(1, 1, 1, 2), text: '' }])
      if (position) {
    ...
    logger.debug('[PlaygroundEditor] Editor setup complete (using Monarch tokenizer)')
  }
  ...
  useEffect(() => {
    ...
    if (lspStatus !== 'connected') {
      logger.debug(`[PlaygroundEditor] LSP not ready yet (status: ${lspStatus})`)
      return
    }

    if (!editorRef.current || !monacoRef.current) {
      logger.debug('[PlaygroundEditor] Editor not mounted yet, skipping refresh')
      return
    }
    ...
    setTimeout(async () => {
      const currentLanguage = model.getLanguageId()
      logger.debug('[PlaygroundEditor] Triggering refresh for language:', currentLanguage)
  ...
  useEffect(() => {
    ...
    logger.debug(`[PlaygroundEditor] Syncing diagnostics for ${activeFileUri}:`, fileDiagnostics)
    ...
    const timer = setTimeout(() => {
      ...
      if (client) {
        ...
        client
          ...
          .catch((err) => logger.error('[PlaygroundEditor] Delayed validation failed:', err))
      }
    }, 5000)
        ...
        {activeFile ? (
          activeFile.language === 'markdown' ? (
            ...
            <Editor
              ...
              theme={resolvedTheme === 'dark' ? 'typedown-dark' : 'typedown-light'}
