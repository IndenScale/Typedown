...
export function StaticCodeBlock({
  ...
  useEffect(() => {
    async function doHighlight() {
      const htmls = await Promise.all(lines.map((line) => highlight(line || ' ', 'typedown')))
          ...
          {lines.map((line, i) => {
            ...
            return (
              <div key={i} className="group relative min-h-[1.5rem]">
                <span className={cn('relative z-10 block', isErrorLine && 'red-wave')}>
                  {html ? (
                    <span
                      dangerouslySetInnerHTML={{
                        __html: html.replace(/<pre[^>]*>|<\/pre>|<code>|<\/code>/g, ''),
                      }}
