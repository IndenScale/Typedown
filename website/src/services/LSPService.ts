...
export class LSPService {
  ...
  public async initialize(): Promise<{
    ...
    if (window.__GlobalLSPClient && window.__GlobalLSPWorker) {
      logger.debug('[LSPService] Global client already active. Reusing instance.')
      return {
    ...
    if (window.__GlobalInitPromise) {
      logger.debug('[LSPService] Initialization already in progress. Waiting...')
      ...
      throw new Error('Concurrent initialization completed but client/worker are missing.')
    }
  ...
  private loadCurrentProject(client: MonacoLanguageClient) {
    ...
    logger.debug(`[LSPService] Loading project with ${filesPayload.length} files.`)
