...
async function initPyodide() {
  ...
  try {
    ...
    if (!wheelResp.ok) throw new Error(`Failed to fetch wheel: ${wheelResp.status}`)
    ...
    if (!scriptResp.ok) throw new Error(`Failed to fetch lsp-server.py: ${scriptResp.status}`)
