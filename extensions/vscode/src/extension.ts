...
export async function activate(context: vscode.ExtensionContext) {
  ...
  const restartCommand = vscode.commands.registerCommand('typedown.restartLsp', async () => {
    if (client) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('typedown.triggerScopeInspection', async () => {
...
async function startServer(context: vscode.ExtensionContext) {
  ...
  const args = config.get<string[]>('server.args') || ['run', '--extra', 'server', 'td', 'lsp']
  ...
  if (command === 'uv' && foundVenvBin) {
    ...
    outputChannel.appendLine(`[Info] Using command: ${finalCommand} ${finalArgs.join(' ')}`)
  }
  ...
  client = new LanguageClient('typedown', 'Typedown Language Server', serverOptions, clientOptions)

  client.onDidChangeState((event) => {
    outputChannel.appendLine(`[Info] Client state change: ${event.oldState} -> ${event.newState}`)
