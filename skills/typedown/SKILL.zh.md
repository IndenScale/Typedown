...
- **ID**：
  - 冒号后的字符串是实体的标识符。
  - **风格**：可以是简单的名称（`alice`）或 Slug 风格（`user-alice-v1`）。
- **内容**：符合 Pydantic 模型的有效 YAML。
  - **引用规则**：在实体块内引用其他实体时，**必须**使用 `[[ID]]` (Wiki Link) 语法。
...
- **规则**：优先使用 **Content Hash** 作为 `former` 指针，以确保历史稳定性。也可以使用全局唯一的 ID。
