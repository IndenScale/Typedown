...
Typedown 的核心理念通过 **脚本 (Scripts)** 将静态文档转化为可执行的智能单元。这是 Typedown 运行时的第一公民（First-Class Citizen）。

## 1. 核心机制

脚本系统允许在 `.td` 文件的 Front Matter 中定义操作逻辑。这使得每个文件不仅是数据的载体，更是拥有行为的**对象**。
...
1. **File Scope**: 当前文件的 Front Matter。优先级最高。
2. **Directory Scope**: 当前目录（及父目录）的 `config.td`。通常用于定义该目录下所有文件的通用行为（如特定的 Build 流程）。
