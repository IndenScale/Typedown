...
def get_typedown_binary_config() -> dict:
    ...
    # Priority: uv tool run > uv run > td direct
    import shutil
...
def setup_zed():
    ...
    typer.echo(f"✅ Zed integration configured successfully!")
    typer.echo(f"")
    typer.echo(f"Settings written to: {settings_path}")
    typer.echo(f"")
    typer.echo(f"Configuration:")
    typer.echo(f"  - LSP Server: typedown")
    typer.echo(f"  - Binary: {binary_config['path']} {' '.join(binary_config['arguments'])}")
    typer.echo(f"  - File types: .td, .typedown → Markdown")
    typer.echo(f"")
    typer.echo(f"Next steps:")
    typer.echo(f"  1. Restart Zed or run 'zed: reload settings'")
    typer.echo(f"  2. Open a .td file to start using Typedown")
...
def zed(
    ...
    if dry_run:
        ...
        typer.echo(f"  File types: .td, .typedown -> Markdown")
        typer.echo(f"  Language servers: typedown added to Markdown")
        return
...
def status():
    ...
    if zed_path and zed_path.exists():
        try:
            ...
            if has_typedown:
                ...
                typer.echo(f"   Binary: {binary.get('path', 'unknown')} {' '.join(binary.get('arguments', []))}")
            else:
                typer.echo("🟡 Zed: Settings exist but Typedown not configured")
                typer.echo(f"   Run: typedown setup zed")
        except Exception:
    ...
    if os.name == 'nt':
        ...
        vscode_settings_dir = Path.home() / "Library" / "Application Support" / "Code" / "User"
